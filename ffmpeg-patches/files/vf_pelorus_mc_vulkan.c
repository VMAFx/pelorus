                         ...
                         const uint32_t *sad, int nblocks)
{
    ...
    sec.data = &mo;
    sec.size = (uint32_t)sizeof(mo);

    if (pel_blob_pack(&meta, &sec, 1, &blob, &len) != PEL_OK || !blob)
        return AVERROR(ENOMEM);

    /* The MV grid (int16 (dx,dy) per block) is appended after the packed blob
     * (the analyze map-payload convention). Grow the buffer, fill the grid, and
     * patch the section's mv_field_offset/size to point at it. The section
     * struct lives at dir[0].offset; mv_field_offset is blob-relative. */
    {
        size_t grid_bytes = (size_t)nblocks * 2 * sizeof(int16_t);
        size_t uuid = PELORUS_SIDEDATA_UUID_LEN;
        uint8_t *grown = av_realloc(blob, len + grid_bytes);
        PelorusSideData *hdr;
        PelorusSectionDir *dir;
        PelorusMotionSection *mo_in_blob;
        uint32_t blob_rel_off;
        ...
        /* The flat blob image starts after the 16-byte UUID prefix. */
        hdr = (PelorusSideData *)(blob + uuid);
        dir = (PelorusSectionDir *)(blob + uuid + hdr->header_size);
        /* Single section: dir[0] is PEL_SEC_MOTION. */
        mo_in_blob = (PelorusMotionSection *)(blob + uuid + dir[0].offset);

        /* Grid goes at the current end of the blob image (total_size), i.e.
         * right after the section payload. blob-relative offset excludes UUID. */
        blob_rel_off = hdr->total_size;
        grid_dst = blob + uuid + blob_rel_off;
        for (i = 0; i < nblocks; i++) {
        ...
        mo_in_blob->mv_field_offset = blob_rel_off;
        mo_in_blob->mv_field_size = (uint32_t)grid_bytes;

        /* Update the framing sizes so a consumer reads the grid as part of the
         * blob (total_size and the dir entry's size stay self-describing — the
         * grid is map payload referenced by offset, not part of the section
         * struct, exactly like analyze's per-cell maps). */
        hdr->total_size += (uint32_t)grid_bytes;
        len += grid_bytes;
    }
