 ...
 *
 * Causal, one-output-per-input: each input frame yields exactly one output. The
 * filter keeps a ring of up to PEL_DENOISE_MAX_PREV previous frames as cheap
 * av_frame_clone refcount bumps on the hwframe images (no pixel copy). No
 * 1-frame latency, no EOF flush.
 *
...
static av_cold int init_filter(AVFilterContext *ctx)
{
    ...
    /* Descriptor set 0. Inputs FIRST, output LAST (the Nin / bwdif binding-order
     * contract): binding 0 = current frame, 1..MAX_PREV = previous frames, then
     * the residual SSBO, then the output image LAST. Each image binding is a
     * per-plane array (.elems = planes), float representation, storage image
     * (sampler = VK_NULL_HANDLE, GENERAL layout, imageLoad). */
    {
 ...
 * residual SSBO and the output via the hand-rolled explicit exec path, dispatch,
 * and (when meta) submit+wait reading the residual back. Inputs first, output
 * LAST. Unused prev slots are bound to the current frame as harmless filler so
 * every array descriptor is populated; the shader only reads 1..actual_prev. */
...
static int denoise_vulkan_filter_frame(AVFilterLink *link, AVFrame *in)
{
    int err;
    int i;
    AVFrame *out = NULL;
    ...
    RET(denoise_dispatch(s, out, in, prev, actual_prev, &acc, &buf));
    ...
    /* Push the current frame into the ring (cheap clone — refcount bump on the
...
static const AVFilterPad pelorus_denoise_vulkan_inputs[] = {
    {
        ...
        .filter_frame = &denoise_vulkan_filter_frame,
