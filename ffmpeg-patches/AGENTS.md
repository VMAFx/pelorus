...
- Filters `require_pkg_config libpelorus` and are gated `vulkan spirv_library`.
- Apply with `git am --3way`; the stack is cumulative.

## Rebase-sensitive invariants

1. **Registration touches three files**: `configure` (`*_filter_deps` +
   `require_pkg_config libpelorus`), `libavfilter/Makefile` (the `OBJS-` line
