...
| [0113](0113-optical-flow-mc.md) | vf_pelorus_mc — GPU motion estimation for MC-temporal denoise (+ gated NVENC ME hints) | Accepted |
| [0114](0114-encoder-steering.md) | Encoder-steering strategy — feed Pelorus GPU maps to every encoder (ROI today, QP-map patches, Vulkan quant-map) | Accepted |
...
| [0134](0134-denoise-shared-mem-tile.md) | Shared-memory tiling of the denoise NLM spatial window (opt-in `tile`, default off): the kernel is fetch-bound (~441 image loads/px re-reading an ~81-px window 9×), so tile=1 cooperatively loads the window into shared memory once per workgroup. Bit-identical (SSIM 1.0). Validated +65.7% (2.9×) on Arc A380, neutral on AMD, −1–4% on the cache-rich 4090 → default off, flagship-first. Supersedes the reverted fp16 attempt (memory-bound, not ALU-bound) | Accepted |
...
| [0139](0139-dehalo-shared-mem-tile.md) | Shared-memory tiling of the dehalo box-blur window (opt-in `tile`, default off): box_blur is fetch-bound (~1445 loads/px from a 5×-called 17×17 window, trivial ALU — an ALU-strip cut Arc rtime −70%), so the ADR-0134 denoise-tiling clone applies. Bit-identical (SSIM 1.0). Validated −38% (1.6×) on Arc A380, neutral on the cache-rich 4090 → default off. Twin of the refuted aa tiling (ALU-bound) | Accepted |
...
| [0146](0146-qsv-roi-frame-ownership.md) | QSV dense ROI maps are per-frame and limited to progressive HEVC CQP; corrects ADR-0114's QSV capability/lifetime detail | Accepted |
